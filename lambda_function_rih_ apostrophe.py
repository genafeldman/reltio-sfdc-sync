...
import numpy as np
import copy  # Added for deep copy fix
    ...
    for contract in full_df['SBQQ__Contract__c'].drop_duplicates():
        ...
        for root_id in temp_df['SBQQ__RootId__c'].drop_duplicates():
            if root_id not in mult_rootid_list:
                ...
                # FIX: Use deep copy instead of fragile string conversion
                # This properly handles apostrophes in product names like "Moody's Orbis"
                base_package_list_full_temp.append(copy.deepcopy(base_package_list_full[0]))
